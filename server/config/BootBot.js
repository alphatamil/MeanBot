...
class BootBot extends EventEmitter {
  ...
  constructor(options) {
    ...
    if (
      ...
      throw new Error(
        "You need to specify an accessToken, verifyToken and appSecret"
      );
    }
    ...
    this.webhook =
      this.webhook.charAt(0) !== "/" ? `/${this.webhook}` : this.webhook;
    this.app.use(
      this.webhook,
  ...
  start(port) {
    ...
    this.server = this.app.listen(this.app.get("port"), () => {
      console.log(`Facebook Webhook running on ${this.webhook}`);
    });
  }
  ...
  sendGenericTemplate(recipientId, elements, options) {
    ...
    options &&
      ...
      (payload.image_aspect_ratio = options.imageAspectRatio) &&
      delete options.imageAspectRatio;
    return this.sendTemplate(recipientId, payload, options);
  }
  ...
  sendListTemplate(recipientId, elements, buttons, options) {
    ...
    options &&
      options.topElementStyle &&
      (payload.top_element_style = options.topElementStyle) &&
      delete options.topElementStyle;
    buttons &&
      buttons.length &&
      (payload.buttons = this._formatButtons([buttons[0]]));
    return this.sendTemplate(recipientId, payload, options);
  }
  ...
  sendRequest(body, endpoint, method) {
    ...
    return fetch(
      ...
      }/me/${endpoint}?access_token=${this.accessToken}`,
      {
        method,
        headers: {
  ...
  sendTypingIndicator(recipientId, milliseconds) {
    ...
    if (milliseconds > 20000) {
      milliseconds = 20000;
      console.error(
        "sendTypingIndicator: max milliseconds value is 20000 (20 seconds)"
      );
    }
  ...
  getUserProfile(userId) {
    const url = `https://graph.facebook.com/${
      this.graphApiVersion
    }/${userId}?fields=first_name,last_name,profile_pic,locale,timezone,gender&access_token=${
      this.accessToken
    }`;
    return fetch(url)
      .then(res => res.json())
